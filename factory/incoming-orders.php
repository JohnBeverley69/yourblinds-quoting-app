...
?>
<style>
    .io-head-row { display: flex; align-items: baseline; gap: 0.75rem; flex-wrap: wrap; margin: 0 0 1.2rem; }
    ...
    .io-sub { color: var(--text-muted, #667); margin: 0 0 1.4rem; }
    .io-order { background: var(--bg-card, #fff); border: 1px solid var(--border, #e5e7eb); border-radius: 12px; margin: 0 0 1rem; overflow: hidden; box-shadow: 0 1px 2px rgba(0,0,0,0.04); }
    .io-head { display: flex; flex-wrap: wrap; align-items: baseline; gap: 0.4rem 1.1rem; padding: 0.8rem 1rem; border-bottom: 1px solid var(--border, #e5e7eb); background: var(--bg-subtle, #f8fafc); }
    .io-head .ref { font-weight: 700; font-variant-numeric: tabular-nums; }
    .io-head .tenant { font-weight: 600; }
    .io-head .meta { color: var(--text-muted, #667); font-size: 0.875rem; }
    ...
    .io-lines th { text-align: left; font-size: 0.7rem; letter-spacing: 0.05em; text-transform: uppercase; color: var(--text-faint, #94a3b8); font-weight: 600; padding: 0.5rem 1rem; border-bottom: 1px solid var(--border, #e5e7eb); }
    .io-lines td { padding: 0.5rem 1rem; border-bottom: 1px solid var(--border, #e5e7eb); font-size: 0.9375rem; vertical-align: top; }
    ...
    ?>
        <section class="io-order<?= $stage === 'dispatched' ? ' is-done' : '' ?>">
            ...
            <table class="io-lines">
                <thead>
                    <tr><th>Product</th><th>Fabric / Colour</th><th class="num">W&times;D (mm)</th><th class="num">Qty</th><th>Room</th></tr>
                </thead>
                <tbody>
                    ...
                    ?>
                        <tr>
                            <td class="prod">
                                ...
                                <?php if ($sys !== ''): ?><br><span class="meta" style="font-weight:400;font-size:0.8125rem;color:var(--text-muted,#667)"><?= e($sys) ?></span><?php endif; ?>
                            </td>
