...
/* ---- Marshalling (Phase D) ------------------------------------------------
 * A finished blind is scanned into the collection/dispatch bench — "marshalled".
 * An order can only be dispatched once every one of its blinds is marshalled, so
 * all the areas' parts physically come together first. All guarded on the
 * marshalled_at column so nothing breaks before migrate_marshalling.php runs. */

/** True once the marshalling column exists. Cached per request. */
function bj_marshal_ready(PDO $pdo): bool
{
    static $ready = null;
    if ($ready !== null) return $ready;
    try { $pdo->query('SELECT marshalled_at FROM factory_blind_jobs LIMIT 0'); return $ready = true; }
    catch (Throwable $e) { return $ready = false; }
}

/**
 * Mark a finished blind marshalled (scanned into collection). It must be made
 * first — you can't collect a blind that isn't finished.
 * @return array{ok:bool, title:string, detail:string, already?:bool}
 */
function bj_mark_marshalled(PDO $pdo, int $itemId, int $unitNo, ?int $userId): array
{
    $q = $pdo->prepare(
        'SELECT bj.id, bj.status, bj.marshalled_at, qi.product_name_snapshot, qi.width_mm, qi.drop_mm,
                qi.quantity, qi.line_no, q.quote_number
           FROM factory_blind_jobs bj
           JOIN quote_items qi ON qi.id = bj.quote_item_id
           JOIN quotes q       ON q.id = bj.quote_id
          WHERE bj.quote_item_id = ? AND bj.unit_no = ? LIMIT 1'
    ...
    if ($job['status'] !== 'complete') {
        return ['ok' => false, 'title' => 'Not made yet', 'detail' => $ref . ' — finish it on the floor before collecting · ' . $what];
    }
    if (!empty($job['marshalled_at'])) {
        return ['ok' => true, 'already' => true, 'title' => $ref, 'detail' => 'already collected · ' . $what];
    }
    $pdo->prepare('UPDATE factory_blind_jobs SET marshalled_at = NOW(), marshalled_by = ? WHERE id = ?')
        ->execute([$userId, (int) $job['id']]);
    return ['ok' => true, 'title' => $ref, 'detail' => 'collected · ' . $what];
}

/** Undo a marshalling scan (put a blind back as not-collected). */
function bj_unmarshal(PDO $pdo, int $jobId, ?int $userId): void
{
    try { $pdo->prepare('UPDATE factory_blind_jobs SET marshalled_at = NULL, marshalled_by = NULL WHERE id = ?')->execute([$jobId]); }
    catch (Throwable $e) { /* column absent — ignore */ }
}

/**
 * Per-order marshalling tally: [quoteId => ['total'=>, 'made'=>, 'marshalled'=>]].
 * total = all floor blinds; made = complete; marshalled = collected.
 */
function bj_order_marshalling(PDO $pdo, array $quoteIds): array
{
    $quoteIds = array_values(array_filter(array_map('intval', $quoteIds)));
    if (!$quoteIds || !bj_marshal_ready($pdo)) return [];
    $ph = implode(',', array_fill(0, count($quoteIds), '?'));
    $st = $pdo->prepare(
        "SELECT quote_id, COUNT(*) AS total,
                SUM(status = 'complete') AS made,
                SUM(marshalled_at IS NOT NULL) AS marshalled
           FROM factory_blind_jobs WHERE quote_id IN ($ph) GROUP BY quote_id"
    );
    $st->execute($quoteIds);
    $out = [];
...
/**
 * Is every floor blind on an order marshalled? True also when the order has no
 * floor blinds at all (nothing to collect — e.g. a pure bought-in order), so this
 * only ever *adds* a gate for orders that actually have blinds. Fails safe: if the
 * column isn't migrated yet it returns true (no marshalling gate).
 */
function bj_all_marshalled(PDO $pdo, int $quoteId): bool
{
    if (!bj_marshal_ready($pdo)) return true;
    try {
        $st = $pdo->prepare(
            'SELECT COUNT(*) AS total, SUM(marshalled_at IS NOT NULL) AS m
               FROM factory_blind_jobs WHERE quote_id = ?'
        );
        $st->execute([$quoteId]);
        $r = $st->fetch(PDO::FETCH_ASSOC) ?: ['total' => 0, 'm' => 0];
        $total = (int) $r['total'];
        return $total === 0 || $total === (int) $r['m'];
