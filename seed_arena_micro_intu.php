...
/**
 * Seeder: Arena Micro INTU — width×drop, TWO mount systems (Screwfit p245,
 * Beadfit p246) × THREE fabric-range bands (Solstice, Hive, Ribbons-Zara).
 * Each grid in the price list is keyed by fabric range, so the range == band:
 *   Solstice     = Solstice asc Micro
 *   Hive         = Hive Micro
 *   Ribbons-Zara = Ribbons asc + Zara Micro
 * Fabrics + colours from the PLT configurator (product type IPM = Intu Pleated
 * Micro). Infusion asc Micro (IPMIFSM) has no price grid in the list → band
 * 'TBC' + inactive.
 *
 * Data: seed_data/arena_micro_intu_prices.csv  (system,band,w,d,price)
 *       seed_data/arena_micro_intu_fabrics.csv (name,colour,band)
 * Idempotent by name, transactional. Run via web: /seed_arena_micro_intu.php
 */
...
$PRODUCT_NAME='Arena Micro INTU'; $SYSTEMS=['Screwfit','Beadfit']; $VALID=['Solstice'=>1,'Hive'=>1,'Ribbons-Zara'=>1];
$DATA=__DIR__.'/seed_data'; $PRICES="$DATA/arena_micro_intu_prices.csv"; $FABRICS="$DATA/arena_micro_intu_fabrics.csv";
...
try {
    ...
    $tc=0;$cc=0;
    foreach($grp as $k=>$cells){ [$s,$b]=explode('|',$k,2); if(!isset($sysId[$s]))continue; $ti->execute([$clientId,$productId,$sysId[$s],$b,"Arena $s $b"]);$tid=(int)$pdo->lastInsertId();
