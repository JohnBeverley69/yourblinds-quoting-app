...
/**
 * Seeder: Arena Faux Wood Venetian — Phase 1. Separate product from metal
 * and real-wood venetians (boss's call).
 *
 * The "Infusion" faux-wood collection — one product, banded A–D where the
 * band IS the range (from price list p43-44):
 *   A = Essence, B = Urban, C = Expressions,
 *   D = Expressions (Highshine White & Cool White only — premium whites)
 * One grid covers all slat widths (38/50/63mm), so a single "Standard"
 * system. The "fabric" is the slat COLOUR (option_label "Colour"), banded
 * by range (colours pre-banded in the CSV); shared on the one system.
 *
 * Data: seed_data/arena_faux_wood_prices.csv  (system,band,w,d,price)
 *       seed_data/arena_faux_wood_colours.csv (system,name,band,range)
 * Idempotent by name, transactional. Run via web: /seed_arena_faux_wood_venetian.php
 */
...
$PRODUCT_NAME = 'Arena Faux Wood Venetian';
$SYSTEMS      = ['Standard'];
$DATA_DIR     = __DIR__ . '/seed_data';
$PRICES_CSV   = $DATA_DIR . '/arena_faux_wood_prices.csv';
$COLOURS_CSV  = $DATA_DIR . '/arena_faux_wood_colours.csv';
...
try {
    ...
    // Colours — band pre-assigned by range in the CSV. system_id NULL.
