...
/**
 * Seeder: Arena Sherwood Venetian — Phase 1. Real-wood (Reelwood) venetian,
 * a separate product from the other wood/faux/metal venetians.
 *
 * One product, one "50mm" system, a single un-banded price grid (price list
 * p23, band_code "STD"). The "fabric" is the wood COLOUR (option_label
 * "Colour"), 16 Reelwood colours, all on the one grid.
 *
 * Data: seed_data/arena_sherwood_prices.csv  (system,band,w,d,price)
 *       seed_data/arena_sherwood_colours.csv (system,name,band)
 * Idempotent by name, transactional. Run via web: /seed_arena_sherwood.php
 */
...
$PRODUCT_NAME = 'Arena Sherwood Venetian';
$SYSTEMS      = ['50mm'];
$DATA_DIR     = __DIR__ . '/seed_data';
$PRICES_CSV   = $DATA_DIR . '/arena_sherwood_prices.csv';
$COLOURS_CSV  = $DATA_DIR . '/arena_sherwood_colours.csv';
...
try {
    ...
    foreach ($grouped as $key => $cells) {
        ...
        $tblIns->execute([$clientId,$productId,$sysId[$sysName],$band,"Arena Sherwood"]); $tid=(int)$pdo->lastInsertId();
        $cseen=[];
    ...
    $ops[] = "Built {$tc} price table(s) ({$cc} cells)."; echo end($ops) . "\n";
    ...
    $ops[] = "Imported {$added} colour options."; echo end($ops) . "\n";
    $pdo->commit();
