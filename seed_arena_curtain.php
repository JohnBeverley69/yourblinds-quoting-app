...
/**
 * Seeder: Arena Curtains — Phase 1. Mirrors seed_arena_roller.php.
 *
 * One product "Arena Curtains" with six HEADING systems, priced in pairs
 * (each pair shares one A–E grid from the May-2026 list):
 *   Pencil Pleat + Deep Pencil Pleat   (p193)
 *   Double Pinch Pleat + Triple Pinch  (p194)
 *   Eyelet + Wave                      (p195)
 * Fabrics are banded from the Curtains Specification (p200-202,
 * "Arena Name | Price Table") and shared across every heading (system_id
 * NULL). Discontinued / tie-back / cushion rows are filtered out upstream.
 *
 * (Per-heading fabric availability — the Y/N columns on the spec — is a
 * Phase-2 refinement; for now every banded fabric shows on all headings.)
 *
 * Data: seed_data/arena_curtain_prices.csv, seed_data/arena_curtain_fabrics.csv
 * Prices = Arena TRADE cost. Idempotent by name, transactional.
 * Run via web (super-admin): /seed_arena_curtain.php
 */
...
$PRODUCT_NAME = 'Arena Curtains';
$OPTION_LABEL = 'Fabric';
$SUPPLIER     = 'Arena';
$SYSTEMS      = ['Pencil Pleat', 'Deep Pencil Pleat', 'Double Pinch Pleat', 'Triple Pinch Pleat', 'Eyelet', 'Wave'];
$DATA_DIR     = __DIR__ . '/seed_data';
$PRICES_CSV   = $DATA_DIR . '/arena_curtain_prices.csv';
$FABRICS_CSV  = $DATA_DIR . '/arena_curtain_fabrics.csv';
...
try {
    ...
    $ops[] = "Imported " . ($added + $inactive) . " fabric options: {$added} active (banded A–E), {$inactive} inactive.";
    echo end($ops) . "\n";
