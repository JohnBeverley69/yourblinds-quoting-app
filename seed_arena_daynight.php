...
/**
 * Seeder: Arena Day & Night Roller (Dual Shade / Vision zebra blind) —
 * width×drop, one "Standard" system, bands A/B/C (price list p164-165).
 * 10 fabric ranges (Beam, Glow, Vision, Lustre…) harvested from the ROB
 * configurator (PRODUCT_TYPE "Dual shade / Day & Night", code ROE);
 * range→band from the Day & Night spec (p166):
 *   A = Beam, Glow, Illumine   B = Verve, View, Vision
 *   C = Dayspring, Lustre, Muse, Sundown
 *
 * Data: seed_data/arena_daynight_prices.csv  (system,band,w,d,price)
 *       seed_data/arena_daynight_fabrics.csv (name,colour,band)
 * Idempotent by name, transactional. Run via web: /seed_arena_daynight.php
 */
...
$PRODUCT_NAME='Arena Day & Night Roller'; $SYSTEMS=['Standard']; $VALID=['A'=>1,'B'=>1,'C'=>1];
$DATA=__DIR__.'/seed_data'; $PRICES="$DATA/arena_daynight_prices.csv"; $FABRICS="$DATA/arena_daynight_fabrics.csv";
